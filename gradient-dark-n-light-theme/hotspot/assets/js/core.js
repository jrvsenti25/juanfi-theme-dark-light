...
function evaluateChargingButton() {
  ...
  for (var i = 0; i < multiVendoAddresses.length; i++) {
    if (
      multiVendoAddresses[i].vendoIp == vendorIpAddress &&
      !multiVendoAddresses[i].chargingEnable
    ) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (r in rows) {
        ...
        rates = rates + "<div class='flex flex-col justify-center w-full'>";
        rates =
          rates +
          "<div class='text-slate-800 dark:text-slate-200'><span>Rate: </span>";
        rates = rates + columns[0];
        rates = rates + "</div>";
        rates =
          rates +
          "<div class='text-sm font-normal text-slate-400 dark:text-slate-500'><span>Validity: ";
        rates = rates + secondsToDhms(parseInt(columns[3]) * 60);
        rates =
          rates +
          "</span><hr class='border-t !border-purple-300 dark:!border-white/10 mt-2 mb-4' /></div>";
        if (dataRateOption) {
          rates =
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (r in rows) {
        ...
        chargingStation =
          chargingStation +
          "<div class='rholder' style='" +
          style +
          "' row-type='charger-port' target-time='" +
          targetTimestamp +
          "'>";
        chargingStation =
          chargingStation + "<div class='rdata'><span>Name: </span>";
        chargingStation = chargingStation + columns[0];
        chargingStation = chargingStation + "</div>";
        chargingStation =
          chargingStation +
          "<div class='rdata'><span style='color: #a3a7ad'>Status: <span name='portStatus'>-";
        chargingStation = chargingStation + "</span></span></div>";
        chargingStation =
          chargingStation +
          "<div class='rdata'><span style='color: #a3a7ad'>Remaining: <span name='remainTime'>-";
        chargingStation = chargingStation + "</span></span></div>";
        chargingStation =
          chargingStation + "<div class='rdata'><span style='color: #a3a7ad'>";
        chargingStation =
          chargingStation +
          "<button class='btn btn-success' style='display: none' name='useBtn' onClick=\"addChargerTime(" +
          r +
          ", '" +
          columns[0] +
          "',0)\">Avail</button>";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == "true") {
        ...
        if (isMultiVendo) {
          $("#insertCoinModalTitle").html(
            "Please insert the coin on " +
              $("#vendoSelected option:selected").text()
          );
        }
        insertcoinbg.play();
      } else {
